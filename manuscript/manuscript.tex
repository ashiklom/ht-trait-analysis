...
\authors{Alexey N. Shiklomanov\affil{1}, Shawn Serbin\affil{2}, David R. Thompson\affil{3}, Philip G. Broderick\affil{3}, and Benjamin Poulter\affil{1}}
...
To simulate top-of-atmosphere radiance, we used a hybrid approach recently developed by Broderick et al.~\emph{in review} (sRTMnet)that combines an atmospheric radiative transfer model (RTM) based on a computationally efficient successive orders of scattering approximation~\cite<Second Simulation of a Satellite Signal in the Solar Spectrum-Vector, 6SV, version 2.1,>{kotchenova2006validation} with a neural network-based emulator of a more advanced RTM~\cite<MODTRAN,>{berk2017validation}.




For a given combination of surface elevation and atmospheric conditions (aerosol optical depth and water vapor content), we simulated
