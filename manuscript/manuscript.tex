...
We provide additional details on each of these steps below.
...
Following \citeA{thompson2018optimal}, we model top-of-atmosphere (TOA) radiance ($L_{\mathrm{TOA}}$) as a function of incoming solar irradiance ($e_{0}$), surface reflectance ($\rho_{s}$), and three terms describing atmospheric optical properties:
...
\begin{linenomath*}
\begin{equation}
  L_{\mathrm{TOA}} = \frac{e_{0}\cos(\theta_{s})}{\pi}\left( \rho_{a} + \frac{t \rho_{s}}{1 - s \rho_{s}} \right)
\end{equation}
\end{linenomath*}
...
To estimate these quantities, we used a hybrid approach \cite<sRTMnet,>{brodrick2021generalized} that combines
...
For atmospheric conditions and surface elevation, we considered ten combinations identified as representative of global conditions (REF). % TODO
...
and two types of atmospheric profiles (REF): tropical and midlatitude summer (Table~\ref{tab:atmospheres}).  % TODO
...
For atmospheric correction of TOA radiance, we used the Imaging Spectrometer Optimal Fitting (ISOFIT) Python library~\cite{thompson2019optimal,thompson2018optimal}.
Briefly, ISOFIT uses optimal estimation \cite{rodgers2000inverse} to jointly estimate surface hemispherical-directional reflectance and the atmospheric state (water vapor content and total aerosol optical depth) and their uncertainties.
ISOFIT supports several different atmospheric RTMs; for this study, we used the same hybrid sRTMnet approach as described in Section~\ref{subsubsec:radiance}.
