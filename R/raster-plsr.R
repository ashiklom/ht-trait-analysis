...
if (FALSE) {
  ...
  rasterfile <- "data/derived/zz-traitanalysis/neon-vegetation.envi"
  plsr_coefs <- read.csv("data/plsr-coefficients/singh-ea-supplement/PLS_coefficients_Mode_A_pct_N.csv")

  pls_coef_files <- dir_ls("data/plsr-coefficients/singh-ea-supplement",
                           regexp = "PLS_coefficients")

  dat <- read.csv(pls_coef_files[7])
